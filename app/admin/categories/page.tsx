...
export default function CategoriesPage() {
  ...
  async function bulkDelete() {
    ...
    if (!response.ok) return showAuthToast({ message: "Could not delete categories.", type: "error" });
    setCategories(value => value.filter(category => !selected.includes(category.slug))); setSelected([]);
    ...
    <section className="np-workspace">
      ...
      <div className="categories-content">
        ...
        <section className="categories-panel">
          ...
          {selected.length > 0 && <div className="categories-bulk"><strong>{selected.length} categories selected</strong><button onClick={() => bulkStatus("Active")}>Set active</button><button onClick={() => bulkStatus("Draft")}>Set draft</button><button onClick={bulkDelete}>Delete</button><button onClick={() => setSelected([])}>Clear</button></div>}
