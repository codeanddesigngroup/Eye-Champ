...
export default function InventoryPage(){
  ...
  const [inventory,setInventory]=useState<InventoryItem[]>([]),[loading,setLoading]=useState(true),[error,setError]=useState(""),[adjustment,setAdjustment]=useState({mode:"Add",quantity:1,reason:"Stock received"}),[saving,setSaving]=useState(false),[receiveOpen,setReceiveOpen]=useState(false),[receiveProductId,setReceiveProductId]=useState("");
  ...
  const continueReceiving=()=>{const item=inventory.find(product=>product.id===receiveProductId);if(!item)return;setReceiveOpen(false);openAdjust(item)};
  ...
  const exportInventory=(rows:InventoryItem[],name="eye-champ-inventory")=>downloadCsv(name,[{label:"Product",value:item=>item.name},{label:"Variant",value:item=>item.variant},{label:"SKU",value:item=>item.sku},{label:"Category",value:item=>item.category},{label:"Status",value:item=>item.status},{label:"Available",value:item=>item.available},{label:"Committed",value:item=>item.committed},{label:"Incoming",value:item=>item.incoming},{label:"Reorder point",value:item=>item.reorder}],rows);
  return <main className="np-admin inventory-admin">
    ...
    <section className="np-workspace">
      ...
      <div className="inventory-content">
        <div className="inventory-head"><div><p>Catalog</p><h1>Inventory</h1><span>Monitor stock levels and keep your products available.</span></div><div><button onClick={()=>exportInventory(shown)}><Download size={16}/> Export inventory</button><button className="receive-stock" onClick={()=>{setReceiveProductId("");setReceiveOpen(true)}}><ArrowDownToLine size={16}/> Receive stock</button></div></div>
        ...
        <section className="inventory-panel">
          ...
          {selected.length>0&&<div className="inventory-bulk"><strong>{selected.length} products selected</strong><button>Update quantities</button><button>Create transfer</button><button onClick={()=>exportInventory(inventory.filter(item=>selected.includes(item.id)),"eye-champ-selected-inventory")}>Export selected</button><button onClick={()=>setSelected([])}>Clear</button></div>}
          ...
          {!loading&&!error&&<div className="inventory-table"><table><thead><tr><th><input type="checkbox" checked={shown.length>0&&selected.length===shown.length} onChange={event=>setSelected(event.target.checked?shown.map(item=>item.id):[])}/></th><th>Product / variant</th><th>SKU</th><th>Status</th><th>Available</th><th>Committed</th><th>Incoming</th><th>Reorder point</th><th/></tr></thead>
          <tbody>{shown.map(item=><tr key={item.id}><td><input type="checkbox" checked={selected.includes(item.id)} onChange={()=>toggle(item.id)}/></td><td><div className="inventory-product"><span>{item.image?<Image src={item.image} alt="" width={68} height={48}/>:<Boxes size={20}/>}</span><div><strong>{item.name}</strong><small>{item.variant}</small></div></div></td><td><code>{item.sku}</code></td><td><StockBadge status={item.status}/></td><td><div className="quantity-cell"><strong>{item.available}</strong><button onClick={()=>openAdjust(item)}><Plus size={13}/> Adjust</button></div></td><td>{item.committed}</td><td>{item.incoming>0?<span className="incoming">+{item.incoming}</span>:"-"}</td><td>{item.reorder}</td><td></td></tr>)}</tbody></table></div>}
    ...
    {receiveOpen&&<div className="adjust-backdrop" onClick={()=>setReceiveOpen(false)}><section className="adjust-modal" role="dialog" aria-modal="true" aria-labelledby="receive-stock-title" onClick={event=>event.stopPropagation()}><div><h2 id="receive-stock-title">Receive stock</h2><button onClick={()=>setReceiveOpen(false)} aria-label="Close">×</button></div><p>Select the product included in this delivery.</p><label>Product<select value={receiveProductId} onChange={event=>setReceiveProductId(event.target.value)}><option value="">Select a product</option>{inventory.map(item=><option value={item.id} key={item.id}>{item.name} — {item.sku} ({item.available} available)</option>)}</select></label><footer><button onClick={()=>setReceiveOpen(false)}>Cancel</button><button className="save" onClick={continueReceiving} disabled={!receiveProductId}>Continue</button></footer></section></div>}
