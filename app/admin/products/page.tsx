...
import { ArrowLeft, ArrowRight, Download, Filter, Glasses, LayoutGrid, List, Pencil, Plus, Search, SlidersHorizontal, Trash2, X } from "lucide-react";
...
export default function ProductsPage() {
  ...
  useEffect(() => { fetch("/api/admin/products", { cache: "no-store" }).then(async response => { const result = await response.json() as { products?: ApiProduct[]; error?: string }; if (!response.ok) throw new Error(result.error); setProducts((result.products ?? []).map(product => ({ id: product.id, name: product.title, sku: product.sku || "No SKU", type: product.categories?.[0] || "Uncategorized", price: Number(product.price), stock: product.quantity, status: product.status, image: product.media?.[0]?.url || (product.status === "Draft" ? "" : "/images/Browline.webp"), colors: (product.variants ?? []).filter(variant => variant.name.toLowerCase().includes("color")).flatMap(variant => variant.values).map(productColor) }))) }).catch(() => showAuthToast({ message: "Could not load products.", type: "error" })) }, []);
  ...
  return <main className="np-admin pc-admin">
    ...
    <section className="np-workspace"><AdminTopbar onMenuOpen={() => setMenuOpen(true)} />
      <div className="pc-content"><div className="pc-head"><div><p>Catalog</p><h1>Products</h1><span>Manage products, inventory, pricing, and visibility.</span></div><div><button onClick={() => downloadCsv("eye-champ-products", [{label:"Product",value:p=>p.name},{label:"SKU",value:p=>p.sku},{label:"Category",value:p=>p.type},{label:"Price",value:p=>p.price},{label:"Stock",value:p=>p.stock},{label:"Status",value:p=>p.status}], shown)}><Download size={16} /> Export</button><Link href="/admin/products/new"><Plus size={17} /> Add product</Link></div></div>
        ...
        <section className="pc-catalog"><div className="pc-tabs"><div><button className={status === "All statuses" ? "active" : ""} onClick={() => setStatus("All statuses")}>All <span>{products.length}</span></button><button className={status === "Active" ? "active" : ""} onClick={() => setStatus("Active")}>Active <span>{products.filter(product => product.status === "Active").length}</span></button><button className={status === "Draft" ? "active" : ""} onClick={() => setStatus("Draft")}>Draft <span>{products.filter(product => product.status === "Draft").length}</span></button><button className={status === "Archived" ? "active" : ""} onClick={() => setStatus("Archived")}>Archived <span>{products.filter(product => product.status === "Archived").length}</span></button></div></div>
          <div className="pc-tools"><label><Search size={16} /><input value={query} onChange={e => setQuery(e.target.value)} placeholder="Search by product name or SKU" /></label><select value={status} onChange={e => setStatus(e.target.value)}><option>All statuses</option><option>Active</option><option>Draft</option><option>Archived</option></select><select><option>All categories</option>{[...new Set(products.map(product => product.type))].map(type => <option key={type}>{type}</option>)}</select><div className="pc-view"><button className={view === "table" ? "active" : ""} onClick={() => setView("table")} aria-label="Table view"><List size={17} /></button><button className={view === "grid" ? "active" : ""} onClick={() => setView("grid")} aria-label="Grid view"><LayoutGrid size={17} /></button></div></div>
          ...
          {view === "table" ? <div className="pc-tablewrap"><table><thead><tr><th><input type="checkbox" checked={shown.length > 0 && selected.length === shown.length} onChange={e => setSelected(e.target.checked ? shown.map(p => p.id) : [])} /></th><th>Product</th><th>Status</th><th>Inventory</th><th>Type</th><th>Price</th><th>Colors</th><th /></tr></thead><tbody>{shown.map(p => <tr key={p.id}><td><input type="checkbox" checked={selected.includes(p.id)} onChange={() => toggle(p.id)} /></td><td><div className="pc-product"><span>{p.image && <Image src={p.image} alt="" width={70} height={52} />}</span><div><Link href={`/admin/products/${p.id}`}><strong>{p.name}</strong></Link><small>{p.sku}</small></div></div></td><td><Status value={p.status} /></td><td><Stock value={p.stock} /></td><td>{p.type}</td><td><strong>Rs {p.price.toFixed(2)}</strong></td><td><div className="pc-colors">{p.colors.map(c => <i style={{ background: c }} key={c} />)}</div></td><td><Link className="pc-edit-link" href={`/admin/products/${p.id}/edit`} aria-label={`Edit ${p.name}`}><Pencil size={16} /></Link></td></tr>)}</tbody></table></div> : <div className="pc-grid">{shown.map(p => <article key={p.id}><label><input type="checkbox" checked={selected.includes(p.id)} onChange={() => toggle(p.id)} /></label><Link className="more pc-edit-link" href={`/admin/products/${p.id}/edit`} aria-label={`Edit ${p.name}`}><Pencil size={16} /></Link><div className="photo">{p.image && <Image src={p.image} alt={p.name} width={240} height={160} />}</div><div className="info"><div><Status value={p.status} /><small>{p.sku}</small></div><h2><Link href={`/admin/products/${p.id}`}>{p.name}</Link></h2><p>{p.type}</p><footer><strong>Rs {p.price.toFixed(2)}</strong><Stock value={p.stock} /></footer></div></article>)}</div>}
