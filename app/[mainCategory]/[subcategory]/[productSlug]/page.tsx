...
export default function DatabaseProductPage({params}:{params:Promise<{mainCategory:string;subcategory:string;productSlug:string}>}){
  ...
  useEffect(()=>{fetch(`/api/products/detail/${encodeURIComponent(productSlug)}`,{cache:"no-store"}).then(async response=>{const result=await response.json() as {product?:DatabaseProduct;error?:string};if(response.status===404){setMissing(true);return;}if(!response.ok||!result.product)throw new Error(result.error||"Product not found.");setProduct(result.product)}).catch(reason=>setError(reason instanceof Error?reason.message:"Could not load product.")).finally(()=>setLoading(false))},[productSlug]);
  if(missing)notFound();
