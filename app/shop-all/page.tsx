...
export default function ShopAll({categorySlug="",subcategorySlug="",catalogTitle="The A-List Collection",gender=""}:{categorySlug?:string;subcategorySlug?:string;catalogTitle?:string;gender?:"Men"|"Women"|""}) {
  ...
  useEffect(() => { const params=new URLSearchParams();if(categorySlug){params.set("category",categorySlug);params.set("subcategory",subcategorySlug)}if(gender)params.set("gender",gender);const query=params.size?`?${params.toString()}`:""; fetch(`/api/products${query}`, { cache: "no-store" }).then(async response => { const result = await response.json() as { products?: Product[]; error?: string }; if (!response.ok) throw new Error(result.error); setProducts(result.products ?? []) }).catch(error => setLoadError(error instanceof Error ? error.message : "Could not load products.")).finally(() => setLoading(false)) }, [categorySlug,subcategorySlug,gender]);
